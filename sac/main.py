...
def main():
    ...
    parser.add_argument('--num_steps', type=int, default=125000, metavar='N',
    ...
    parser.add_argument('--sim_dt', required=False, type=float, default='0.002')
    args = parser.parse_args()

    # sim_dt = 0.002

    # dt-aware
    # args.gamma = args.gamma ** (args.act_dt / 0.016)
    args.gamma = args.gamma ** (args.g_exp)
    args.replay_size = int(args.replay_size * (0.016 / args.act_dt))

    env_dir_id = 'invdblpndlm4' if args.env_name == 'HighFreqInvertedDoublePendulumBulletEnv-v0' else 'reacher'
    ...
    args.num_steps = args.num_steps * int(0.016 // args.sim_dt)
    args.start_steps = args.start_steps * int(0.016 // args.sim_dt)
    ...
    for i_episode in itertools.count(1):
        ...
        while not done:
            ...
            if (episode_steps + 1) % ac_interval == 0 or done:
                ...
                mask = 1 if episode_steps + 1 == env._max_episode_steps else float(not done)
        ...
        updated_agent.save_model(env_name='highfreqreacher', actor_path=weights_path_actor, critic_path=weights_path_critic)
