    ...
    def _compute_sensation_(self, name, sensor_window, timestamp_window, index_window):
        ...
        if self._safety_mode_ != ur_utils.SafetyModes.NONE and \
           ...
           not self._in_safety_violation:
            ...
            # with open('logs/s_0.3_a_1.4_v2_learn.txt', 'a', encoding='utf-8') as log_file:
            #     reg_log = {}
            #     for k in sensor_window[0].dtype.fields.keys():
            #         reg_log[k] = [sensor_window[i][k][0].tolist() for i in range(-self._comm_history, 0)]
            #     reg_log['env_q_target'] = self._q_target_.tolist()
            #     reg_log['env_x_target'] = self._x_target_.tolist()
            #     reg_log['env_safety_violation_count'] = self._safety_violation_count
            #     log_file.write('\n' + json.dumps(reg_log) + '\n,\n')
